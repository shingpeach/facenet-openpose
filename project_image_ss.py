...
def process (input_image, params, model_params):
    ...
    for n in range(len(subset)):
        ...
        if any (a > 110 for a in angles):
            ...
            body_ori = oriImg[(sq_Ymin+z):(sq_Ymin+3*z), (sq_Xmin-z):(sq_Xmax+z)]
            # 影像去噪
...
def output(rdd):
    topic = 'dl_ouput'
    producer = KafkaProducer(bootstrap_servers=['localhot:9092'])
    
    for stats in rdd:
        value = json.dumps(stats)
        producer.send(topic, value=bytes(value, 'utf-8'))
    producer.close()
...
if __name__ == '__main__':
    ...
    # res = num.map(lambda x:player_dict[x]).cache()
    # res.pprint()
    # res.foreachRDD(output)
    ...
    info_list,canvas = process(input_image, params, model_params)
