...
def process (input_image, params, model_params):
    ...
    for n in range(len(subset)):
        ...
        if abs(right_shoulder_X - left_shoulder_X) >= 100:
            body_ori = canvas[right_shoulder_Y:, right_shoulder_X:left_shoulder_X]
...
if __name__ == '__main__':
    ...
    frame, list1 = f.infer(input_image)
    print(list1)
    ...
    R,G,B = process(input_image, params, model_params)
    
    team, num, name = list1[0][0].split('_')
    
    with open('player_list.csv', 'w', newline='') as csvFile:   
